...
object LogAnalyseFuns {
  ...
  def getResponseCodesAndFrequencies(data:RDD[Row]):List[(Int,Int)]= {
    data.groupBy(x=>x.getInt(7)).map(x=> (x._1,x._2.toList.length)).collect().toList
  }
  ...
  def getTopTenEndpoints(data:RDD[Row]):List[(String, Int)]= {
    data.groupBy(x=>x.getString(5)).map(y=>(y._1,y._2.toArray.length)).sortBy(z=>z._2,false,4).take(10).toList
    }
  ...
  def top25ErrorCodeResponseHosts(data:RDD[Row]):Set[(String,Int)]= {
    val n = data.groupBy(x=>x.getString(0)).map(y=>(y._1,y._2.groupBy(z=>z.getInt(7)).getOrElse(404, Iterable.empty).toList.length)).sortBy(b=>b._2, false, 4).take(25).toList.toSet
    return n
  }
  ...
  def responseErrorCodesPerDay(data:RDD[Row]):List[(Int,Int)]= {
    ...
    val m = data.groupBy(x=>x.getAs[OffsetDateTime](3).getDayOfMonth).map(y=>(y._1,y._2.groupBy(z=>z.getInt(7)).getOrElse(404, Iterable.empty).toList.length)).sortBy(a=>a._1, true, 4).collect().toList
    return m
  }
  ...
  def errorResponseCodeByHour(data:RDD[Row]):List[(Int,Int)]= {
    val m = data.groupBy(x=>x.getAs[OffsetDateTime](3).getHour).map(y=>(y._1,y._2.groupBy(z=>z.getInt(7)).getOrElse(404, Iterable.empty).toList.length)).sortBy(a=>a._1, true, 4).collect().toList
    return m
  }
  ...
  def getAvgRequestsPerWeekDay(data:RDD[Row]):List[(Int,String)]= {
    ...
    val m = data.groupBy(x=>x.getAs[OffsetDateTime](3).getDayOfWeek).map(y=>(y._2.toList.length / y._2.groupBy(b=>b.getAs[OffsetDateTime](3).getDayOfMonth).keys.toList.length,y._1)).sortBy(z=>z._2.getValue, true, 4).map(a=>(a._1,a._2.toString())).collect().toList
