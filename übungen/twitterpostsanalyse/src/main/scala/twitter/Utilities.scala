   ...
   * 
{
  ...
  // es geht zwar immer um 1 line, warum dann List(Row), da Row eventuell kein Tweet sein muss / ist, dann wird einfach eine Leere Liste returned
  
  
  def parse(line:String):List[Row]= {
    val parsed : Map[String,Any] = (JSON.parseFull(line) match {case Some(a) => a; case None => Map.empty}).asInstanceOf[Map[String,Any]]
    val row = Row(
        parsed.get("created_at").asInstanceOf[OffsetDateTime],
        parsed.get("user").asInstanceOf[Map[String,Any]].getOrElse("name",""),
        parsed.getOrElse("text",""),
        parsed.getOrElse("lang", ""))
    print(List(row))
    return List(row)
  }
